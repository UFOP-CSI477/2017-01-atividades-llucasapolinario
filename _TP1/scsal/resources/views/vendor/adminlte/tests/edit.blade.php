...
<div class="row">
    <div class="col-md-8 col-md-offset-2">
      <form class="form-horizontal" role="form" method="post" action="/tests/{{$test->id}}">
        {{ csrf_field() }}
        {{ method_field('PATCH') }}
        <fieldset>

          <legend>Editar Test</legend>
          ...
          <div class="form-group">
            <label class="col-sm-3 control-label" for="textinput">Id do usuário</label>
            <div class="col-sm-9">
              <input type="date" name="date" value="{{$test->date}}" class="form-control">
            </div>
          </div>
          ...
          <div class="form-group">
            ...
            <span><i aria-hidden="true"></i></span>
              <select class="col-sm-6 form-control" name="procedure_id">
                <option value="{{ $test->procedure_id }}">{{$procedure = App\Procedure::find($test->procedure_id)->name}}</option>
                @foreach($procedures = App\Procedure::all() as $procedure)
